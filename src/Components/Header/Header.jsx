...
export default function Header() {
  ...
  return (
    <div className="container mx-auto py-2 md:py-11 px-4 flex justify-between items-center">
       <div className="md:hidden text-3xl" onClick={()=>setOpen(!open)}>
        {open?  <IoClose></IoClose>: <IoMenu></IoMenu>}
       </div>
        <div>
            <img className="w-18 md:w-24" src="/logo.png" alt="" />
        </div>
        <div>
            <ul className={`md:flex gap-4 text-xl absolute md:static duration-1000 ${open? 'top-12 left-10 bg-[#D9D9D9] p-2 rounded-sm shadow-4xl':'-top-60 left-3'}`}>
                ...
                <li className="hover:font-bold"> <Link>Shop</Link></li>
